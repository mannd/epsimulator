...
void epsimulator::newDoc()
{
    epsimulator *ed = new epsimulator;
    ed->setCaption(tr("Qt Example - Application"));
    ed->show();
}

void epsimulator::choose()
{
...
void epsimulator::load( const QString &fileName )
{
    QFile f( fileName );
...
void epsimulator::closeEvent( QCloseEvent* ce )
{
    if ( !e->isModified() ) {
	ce->accept();
	return;
