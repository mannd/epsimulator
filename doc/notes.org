...
*** General considerations
    When Enable Network Storage is checked in the System Settings
    dialog, the epsimulator.db database used is located in the root dir
    of the Network Study Path.
*** System start
**** Options
     If the epsimulator.db file is present at system start, a the
     createConnection() function in main.cpp connects to it instead of
     the database file in the System dir.  If not present:
       - option 1: create new blank epsimulator.db
       - option 2: copy local [systemdir]/epsimulator.db to [networkdir]/epsimulator.db
       - option 3: merge local [systemdir]/epsimulator.db to [networkdir]/epsimulator.db
       - option 4: quit and suggest manually setting up
         [networkdir]/epsimulator.db
**** Functions needed
     - copy blank database to network
     - copy system database to network
     - merge system database to network
       - if we do above, can we eliminated duplicates?  Maybe merging
         is not appropriate for the System database, although it
         certainly needs to be an option for the Study database.
** Study database
* Study class problems
** path()
   - path() returns path_
   - filePath() returns path_/studies.dat  // WRONG!
   - dirName() returns /{key}  // ?????
   - configFilePath() returns path_/config.dat  // WRONG!
** path problems
   There is ambiguity as to whether path_ is root path of studies dir
   - e.g. ~/.epsimulator/studies
   - or ~/.epsimulator/studies/{key.....}
   Loading and saving need the latter.  Not sure if the former is ever used.
** What Study needs to know
*** needs a root path, that is initialized or set
*** needs a key, which is autogenerated or set
*** that's all!
* Study paths
** Study::load() and Study::save()
   Studies can load and save themselves.  In order to do so, the Study needs
   to know its root path.  This is stored in the path_ data member.
** Study path details
   | variable              | function   | example                                |
   |-----------------------+------------+----------------------------------------|
   | path_                 | path()     | ~/.epsimulator                         |
   | EPSIM_STUDIES_DIRNAME |            | studies                                |
   | fileName_             |            | study.dat                              |
   |                       | filePath() | ~/.epsimulator/studies/{KEY}/study.dat |
   |                       |            |                                        |
